...
Array2D.z超级透视 = function(arr, rowFields, colFields, dataFields, headerRows, outputHeader, separator) {
    ...
    if (outputHeader === 1 || outputHeader === true) {
        ...
        // 🔧 ==================== 多行多列透视表表头设计 ====================
        // 🔧 v3.8.0 增强：支持真正的多级列表头（每列字段占一行）
        //
        // 表头结构：
        // - 第1行：(numRowFieldLevels-1)个空白 + 列字段1标题 + 列字段2标题 + ...
        // - 第2行：(numRowFieldLevels-1)个空白 + 列字段1的值1  + 列字段1的值2 + ...
        // - 第3行：(numRowFieldLevels-1)个空白 + 列字段2的值1  + 列字段2的值2 + ...
        // - ...
        // - 最后一行：行字段标题 + 数据字段标题（每个数据字段重复）
        //
        // 计算层级数
        var numColFieldLevels = colConfig.fields.length;
        var numRowFieldLevels = rowConfig.fields.length;
        var headerRowCount = numColFieldLevels + 1; // 列字段数量 + 1行数据字段标题
        ...
        // ============ 步骤1: 填充行字段区域（左上角）============
        // 对于除最后一行外的所有行，填充行字段占位符
        for (var h = 0; h < headerRowCount - 1; h++) {
        ...
        for (var rfIdx = 0; rfIdx < numRowFieldLevels; rfIdx++) {
            ...
            headerRows[headerRowCount - 1].push(rowTitle);
        }

        // ============ 步骤2: 填充列字段和数据字段区域============
        // 构建列键的层级展开
        // colKeys 是 "值1|||值2|||值3" 格式
        // 需要展开为多行，每行显示一个列字段层级的值
        ...
        for (var cfIdx = 0; cfIdx < numColFieldLevels; cfIdx++) {
            ...
            if (hasColTitles && colConfig.titles[cfIdx]) {
                ...
                if (match) {
                    var origIdx = parseInt(match[1]) - 1;
                    colFieldTitle = _originalHeader[origIdx] || '';
                }
            } else if (arr && arr[0]) {
                var match = colConfig.fields[cfIdx].field.match(/^f(\d+)$/);
                if (match) {
            ...
            // 每个列字段标题横跨该字段所有值的所有数据字段列
            var numUniqueValues = cfIdx === 0 ? colKeys.length :
                                  getUniqueLevelCount(colKeys, cfIdx);
            var span = numUniqueValues * numDataFields;
            for (var s = 0; s < span; s++) {
                headerRows[0].push(colFieldTitle);
            }
        }

        // 生成列字段值行（第2行到第numColFieldLevels行）
        for (var cfIdx = 0; cfIdx < numColFieldLevels; cfIdx++) {
            var currentLevel = cfIdx; // 当前层级 (0-based)
            var targetRow = cfIdx + 1; // 目标行（第1行是标题，所以从2开始）

            // 获取该层级的所有唯一值（按顺序）
            var levelValues = getLevelValues(colKeys, currentLevel);

            // 每个值横跨 numDataFields 列
            for (var v = 0; v < levelValues.length; v++) {
                var val = levelValues[v];
                for (var dfIdx = 0; dfIdx < numDataFields; dfIdx++) {
                    headerRows[targetRow].push(val);
                }
            }
        }
    ...
    // 🔧 DEBUG: 最终结果
    // Console.log('[superPivot DEBUG] 最终结果: ' + result.length + '行');
