...
function test_superPivot_example1() {
    ...
    // 预期输出（修正：行值类型与数据源一致）
    // 注意：由于数据行第一列可能是数字或字符串，superPivot会保持原类型
    // 但当前输出显示为字符串，这是因为从数据源提取时保持了类型
    var expected = [
        ["","价格",1,1,5,5,10,10],
...
function test_superPivot_example2() {
    ...
    // 预期输出（修正）
    // 注意：
    // 1. 行字段有3个（f1,f5,f6），所以前3列是行字段相关
    // 2. 列字段有1个（f2），列字段标题在索引2的位置（numRowFieldLevels-1=2）
    // 3. 实际输出有3个空列（索引0,1,2中的前两个是空，索引2是列字段标题）
    // 4. 使用自定义标题 '计数,求和,平均'
    var expected = [
        ["","","country","product1","product1","product1","product2","product2","product2","product3","product3","product3"],
        ["prod","year","month","计数","求和","平均","计数","求和","平均","计数","求和","平均"],
        ["1","10","2023","","","","","","","",1,0,29],
        ["2","5","2023","","","",1,0,20,"","",""],
        ["3","1","2023",1,0,19,"","","","","",""],
        ["4","10","2023","","","","","","","",1,0,24],
...
function test_superPivot_example3() {
    ...
    // 验证关键字段
    // SuperMap键格式: '01L00001 01L0001 1@^@10@^@2023@^@product3'
    // 说明: superPivot返回的键已经是 '01L0001 xxx' 格式
    //       SuperMap.fromMap 会再添加一层前缀 '01L00001 '
    var expectedKeys = [
        '1@^@10@^@2023@^@product3',
        '2@^@5@^@2023@^@product2',
        '3@^@1@^@2023@^@product1',
    ...
    for (var i = 0; i < expectedKeys.length; i++) {
        ...
        // SuperMap 的双层前缀格式: '01L00001 01L0001 xxx'
        var innerSeq = String(i + 1);
        while (innerSeq.length < 4) {
            innerSeq = '0' + innerSeq;
        }
        var innerKey = '01L' + innerSeq + ' ' + key;

        var outerSeq = String(i + 1);
        while (outerSeq.length < 5) {
            outerSeq = '0' + outerSeq;
        }
        var superMapKey = '01L' + outerSeq + ' ' + innerKey;

        if (rs.all[superMapKey]) {
            Console.log('✅ 键存在: ' + key);
        } else {
            Console.log('❌ 键缺失: ' + key + ' (期望SuperMap键: ' + superMapKey + ')');
            // 输出实际可用的键供调试
    ...
    for (var j = 0; j < expectedKeys.length; j++) {
        ...
        // SuperMap 的双层前缀格式: '01L00001 01L0001 xxx'
        var innerSeq = String(j + 1);
        while (innerSeq.length < 4) {
            innerSeq = '0' + innerSeq;
        }
        var innerKey = '01L' + innerSeq + ' ' + key;

        var outerSeq = String(j + 1);
        while (outerSeq.length < 5) {
            outerSeq = '0' + outerSeq;
        }
        var superMapKey = '01L' + outerSeq + ' ' + innerKey;

        var item = rs.all[superMapKey];
        if (item) {
            ...
            Console.log('❌ 未找到键: ' + key + ' (SuperMap键: ' + superMapKey + ')');
            pass = false;
        }
    }
