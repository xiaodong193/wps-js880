...
/**
 * z合并相同单元格 - 合并指定区域中相同的行或者列（WPS JSA 完全兼容版）
 *
 * 修复：
 * - 使用 sheet.Cells 和 sheet.Range 确保绝对引用
 * - 跳过空值，避免空单元格参与合并
 * - 优化性能，添加 ScreenUpdating 控制
 * - 修复边界检查，防止越界错误
 *
 * @param {Range|string} rng - 要合并的单元格区域
 * @param {string} [direction='r'] - 合并方式: 'r'按行 'c'按列 'rm'先行后列 'cm'列按上下级关系
 * @returns {undefined} 无返回值
 * @example
 * // 列标题按列合并
 * RngUtils.z合并相同单元格("a1:j1","c")
 * // 行标题按层级合并（考虑前一列关系）
 * RngUtils.z合并相同单元格(Range("a3:b16"),"rm")
 */
RngUtils.z合并相同单元格 = function(rng, direction) {
    ...
    try {
        direction = direction || 'r';
        var r = typeof rng === 'string' ? Range(rng) : rng;
        var sheet = r.Worksheet; // ✅ 关键：获取工作表对象用于绝对引用

        if (!sheet) {
            console.error("无法获取工作表对象");
            return;
        }
        ...
        // ✅ 辅助函数：比较两个值是否相等（空值视为相等）
        function isSameValue(val1, val2) {
            if (isEmpty(val1) && isEmpty(val2)) return true;
            if (isEmpty(val1) || isEmpty(val2)) return false; // ✅ 只有一个为空时，不相等
            return String(val1).trim() === String(val2).trim();
        }

        console.log("=== mergeCells 开始 ===");
        console.log("区域:", r.Address());
        console.log("工作表:", sheet.Name);
        console.log("方向:", direction);
        console.log("行数:", r.Rows.Count, "列数:", r.Columns.Count);

        // 计算相对于工作表的绝对位置
        var baseRow = r.Row;
        var baseCol = r.Column;
        ...
        if (direction === 'r' || direction === '-r') {
            ...
            for (var i = 1; i <= r.Rows.Count; i++) {
                ...
                if (isEmpty(startVal)) {
                    ...
                    if (isEmpty(startVal)) continue; // 整行都是空值，跳过
                }

                for (var j = startCol + 1; j <= r.Columns.Count; j++) {
                    var currentVal = r.Cells(i, j).Value2;

                    // ✅ 遇到空值跳过，但不中断当前合并组
                    if (isEmpty(currentVal)) continue;

                    if (!isSameValue(currentVal, startVal)) {
                        if (j - startCol > 1) {
                            var absRow1 = baseRow + i - 1;
                            var col1 = baseCol + startCol - 1;
                            var col2 = baseCol + j - 2;
                            var cell1 = sheet.Cells(absRow1, col1);
                            var cell2 = sheet.Cells(absRow1, col2);
                            var mergeRng = sheet.Range(cell1, cell2);
                            try {
                                mergeRng.Merge();
                                mergeCount++;
                                if (i <= 3) console.log("  -> 合并 行" + absRow1 + " 列" + col1 + "-" + col2);
                            } catch(e) {
                ...
                if (r.Columns.Count - startCol + 1 > 1) {
                    var absRow1 = baseRow + i - 1;
                    var col1 = baseCol + startCol - 1;
                    var col2 = baseCol + r.Columns.Count - 1;
                    var cell1 = sheet.Cells(absRow1, col1);
                    var cell2 = sheet.Cells(absRow1, col2);
                    var mergeRng = sheet.Range(cell1, cell2);
                    try {
            ...
            console.log("按行合并完成，共合并 " + mergeCount + " 次");
        }
        // 2. 按列合并 (c/-c)
            ...
            for (var j = 1; j <= r.Columns.Count; j++) {
                ...
                if (isEmpty(startVal)) {
                    ...
                    if (isEmpty(startVal)) continue; // 整列都是空值，跳过
                }

                for (var i = startRow + 1; i <= r.Rows.Count; i++) {
                    var currentVal = r.Cells(i, j).Value2;
                    ...
                    if (!isSameValue(currentVal, startVal)) {
                        if (i - startRow > 1) {
                            var absCol1 = baseCol + j - 1;
                            var row1 = baseRow + startRow - 1;
                            var row2 = baseRow + i - 2;
                            var cell1 = sheet.Cells(row1, absCol1);
                            var cell2 = sheet.Cells(row2, absCol1);
                            var mergeRng = sheet.Range(cell1, cell2);
                            try {
                                mergeRng.Merge();
                                mergeCount++;
                                if (j <= 3) console.log("  -> 合并 列" + absCol1 + " 行" + row1 + "-" + row2);
                            } catch(e) {
                                console.log("  -> 合并失败:", e.message);
                            }
                        }
                ...
                if (r.Rows.Count - startRow + 1 > 1) {
                    var absCol1 = baseCol + j - 1;
                    var row1 = baseRow + startRow - 1;
                    var row2 = baseRow + r.Rows.Count - 1;
                    var cell1 = sheet.Cells(row1, absCol1);
                    var cell2 = sheet.Cells(row2, absCol1);
                    var mergeRng = sheet.Range(cell1, cell2);
                    try {
            ...
            console.log("按列合并完成，共合并 " + mergeCount + " 次");
        }
        // 3. 按层级合并 (rm/-rm) - 行标题层级合并
        else if (direction === 'rm' || direction === '-rm') {
            console.log("按层级合并（考虑前一列关系）...");
            var mergeCount = 0;
            ...
            for (var j = 1; j <= r.Columns.Count; j++) {
                ...
                for (var i = 1; i <= r.Rows.Count; i++) {
                    var currVal = r.Cells(i, j).Value2;

                    // ✅ 跳过空值
                    if (isEmpty(currVal)) continue;

                    if (prevCol >= 1) {
                        var prevColVal = r.Cells(i, prevCol).Value2;

                        // ✅ 前一列为空或值相同则跳过
                        if (isEmpty(prevColVal) || isSameValue(prevColVal, currVal)) continue;
                    }

                    var startRow = i;
                    var k;
                    for (k = i + 1; k <= r.Rows.Count; k++) {
                        var nextVal = r.Cells(k, j).Value2;

                        // ✅ 遇到空值或不同值则停止
                        if (isEmpty(nextVal) || !isSameValue(nextVal, currVal)) break;

                        // ✅ 检查前一列的层级关系
                        if (prevCol >= 1) {
                            var prevValK = r.Cells(k, prevCol).Value2;
                            var prevValKMinus1 = r.Cells(k - 1, prevCol).Value2;

                            if (!isEmpty(prevValK) && !isSameValue(prevValK, prevValKMinus1)) break;
                        }
                    }
            ...
            console.log("按层级合并完成，共合并 " + mergeCount + " 次");
        }
        // 4. 多级表头按列方向合并 (cm) - 列标题层级合并
        else if (direction === 'cm' || direction === '-cm') {
            console.log("按列方向合并（多级表头）...");
            var mergeCount = 0;
            var rowHeaderCols = 3;

            for (var row = baseRow; row < baseRow + r.Rows.Count; row++) {
                var startCol = baseCol + rowHeaderCols - 1;
                var startVal = sheet.Cells(row, startCol).Value2;

                // ✅ 跳过起始值为空的行
                if (isEmpty(startVal)) {
                    for (var col = startCol + 1; col <= baseCol + r.Columns.Count - 1; col++) {
                        var val = sheet.Cells(row, col).Value2;
                        if (!isEmpty(val)) {
                            startCol = col;
                            startVal = val;
                            break;
                    ...
                    if (isEmpty(startVal)) continue;
                }

                var lastCol = startCol;

                for (var col = startCol + 1; col <= baseCol + r.Columns.Count - 1; col++) {
                    var currentVal = sheet.Cells(row, col).Value2;
                    ...
                    if (!isSameValue(currentVal, startVal)) {
                        if (lastCol > startCol) {
                            var cell1 = sheet.Cells(row, startCol);
                            var cell2 = sheet.Cells(row, lastCol);
                            var mergeRng = sheet.Range(cell1, cell2);
                            try {
                                mergeRng.Merge();
                                mergeCount++;
            ...
            console.log("列方向表头合并完成，共合并 " + mergeCount + " 次");
        }

        console.log("=== mergeCells 结束 ===");
    } catch (e) {
        console.error("mergeCells 发生错误:", e.message);
    } finally {
...
/**
 * z合并透视表表头 - 专用于 superPivot 结果的表头合并
 * 只合并行标题和列标题区域，数据区域不合并
 *
 * WPS JSA 修复版：使用 sheet.Cells 和 sheet.Range 确保绝对引用
 *
 * @param {Range|string} rng - 透视表输出区域
 * @param {Object} [options] - 合并配置选项
 * @param {number} [options.rowHeaderCols=3] - 行标题列数
 * @param {number} [options.colHeaderRows=2] - 列标题行数
 * @param {boolean} [options.mergeRowHeaders=true] - 是否合并行标题
 * @param {boolean} [options.mergeColHeaders=true] - 是否合并列标题
 * @example
 * var rs = Array2D.superPivot(arr, ['f2,f6+,f7'], ['f3'], ['count(),sum("f4")']);
 * var rng = rs.toRange('K2');
 * RngUtils.mergePivotHeaders(rng);
 */
RngUtils.z合并透视表表头 = function(rng, options) {
    if (!isWPS) return;
    ...
    console.log("=== mergePivotHeaders 开始 ===");
    console.log("区域:", r.Address());
    console.log("工作表:", sheet.Name);
    console.log("总行数:", r.Rows.Count, "总列数:", r.Columns.Count);

    // 默认参数
    var rowHeaderCols = options.rowHeaderCols || 3;
    var colHeaderRows = options.colHeaderRows || 2;
    var startRow = r.Row;
    var startCol = r.Column;
    var endRow = startRow + r.Rows.Count - 1;
    var endCol = startCol + r.Columns.Count - 1;

    console.log("使用参数 - 行标题列数:", rowHeaderCols, "列标题行数:", colHeaderRows);

    // ✅ 关键修复：限制合并范围
    // 行标题合并：只在前 colHeaderRows 行内合并
    var rowMergeEndRow = startRow + colHeaderRows - 1;
    // 列标题合并：从行标题列之后开始合并（最后一列行标题是列标题区域）
    var colMergeStartCol = startCol + rowHeaderCols - 1;

    console.log("合并范围 - 行标题合并到第", rowMergeEndRow, "行，列标题合并从第", colMergeStartCol, "列开始");

    // 合并行标题区域（按列纵向合并相同值）- 只在表头行内
    if (options.mergeRowHeaders !== false && rowHeaderCols > 0) {
        console.log("\n--- 合并行标题（按列纵向，仅表头行）---");
        for (var col = startCol; col < startCol + rowHeaderCols; col++) {
            var mergeStart = startRow;
            var currentVal = null;

            // ✅ 只遍历表头行，不合并数据行
            for (var row = startRow; row <= rowMergeEndRow; row++) {
                ...
                if (currentVal === null) {
                    ...
                    if (row - 1 > mergeStart) {
                        sheet.Range(
                            sheet.Cells(mergeStart, col),
                            sheet.Cells(row - 1, col)
                        ).Merge();
                    }
                    currentVal = val;
                    mergeStart = row;
            ...
            // 处理最后一组
            if (rowMergeEndRow > mergeStart) {
                sheet.Range(
                    sheet.Cells(mergeStart, col),
                    sheet.Cells(rowMergeEndRow, col)
                ).Merge();
            }
        }
        console.log("行标题合并完成");
    ...
    // 合并列标题区域（按行横向合并相同值）- 从行标题列之后开始
    if (options.mergeColHeaders !== false && colHeaderRows > 0) {
        console.log("\n--- 合并列标题（按行横向，从列标题区开始）---");
        for (var row = startRow; row < startRow + colHeaderRows; row++) {
            var mergeStart = colMergeStartCol;
            var currentVal = null;

            // ✅ 从行标题列之后开始遍历，避免合并行标题区域
            for (var col = colMergeStartCol; col <= endCol; col++) {
                var cell = sheet.Cells(row, col);
                var val = cell.Value2;
    ...
    console.log("=== mergePivotHeaders 结束 ===");
};
RngUtils.mergePivotHeaders = RngUtils.z合并透视表表头;
...
if (isWPS || isBrowser) {
    (function() {
        // 测试热重载功能 - 2025/01/24 测试
        console.log('JSA880-enhanced.js 已更新 - 热重载测试');
         ...
         * 
         * 使用方法:
         *   在 WPS 中执行: Run("模块名.hotReload")
         *   示例: Run("mJSA880_V1_6_1.hotReload")
         * 
         * 注意:
         *   - 需要先运行 watch-and-reload.js 监控源文件
         *   - 修改源 JS 文件并保存后，xlsm 会自动更新
         *   - 然后调用此函数重新加载新代码
         */
        this.hotReload = function() {
            try {
                // 1. 显示开始提示
                Application.StatusBar = "正在热重载 JSA 模块...";
                
                // 2. 读取重载标志文件
                var flagFile = "/tmp/wps_jsa_reload_flag.txt";
                var fso = new ActiveXObject("Scripting.FileSystemObject");
                
                // 3. 检查是否有更新
                if (!fso.FileExists(flagFile)) {
                    Application.StatusBar = "没有检测到更新";
                    MsgBox("没有检测到代码更新\n请先保存源 JS 文件", 64, "热重载");
                    return false;
                }
                
                // 4. 读取标志文件内容
                var ts = fso.OpenTextFile(flagFile, 1, false);
                var flagContent = ts.ReadAll();
                ts.Close();
                
                var flag = JSON.parse(flagContent);
                
                // 5. 显示更新信息
                var updateDate = new Date(flag.timestamp);
                var timeStr = updateDate.toLocaleString();
                ...
                var result = MsgBox(
                    ...
                    "模块: " + flag.module + "\n\n" +
                    "是否重新加载?",
                    36,  // 36 = vbYesNo + vbQuestion
                    "热重载"
                );
                
                // 6. 用户选择
                if (result != 6) {  // 6 = vbYes
                    Application.StatusBar = "取消重载";
                    return false;
                }
                
                // 7. 清理全局状态（根据你的模块需求）
                if (typeof _JSA880_init !== 'undefined') {
                    _JSA880_init = false;
                }
                
                // 8. 重新初始化模块
                Application.StatusBar = "正在初始化模块...";
                
                // 调用你的初始化函数
                if (typeof initJSA880 !== 'undefined') {
                    initJSA880();
                }
                ...
                // 10. 显示成功提示
                Application.StatusBar = "热重载完成";
                MsgBox("热重载成功！\n\n代码已更新并重新初始化", 64, "成功");
                
                return true;
                
            } catch (e) {
                Application.StatusBar = "热重载失败";
                MsgBox("热重载失败:\n" + e.message, 16, "错误");
                return false;
            }
        };
         ...
         * 直接重载，不显示确认对话框
         */
        this.hotReloadQuick = function() {
            try {
                Application.StatusBar = "快速热重载...";
                    
                var flagFile = "/tmp/wps_jsa_reload_flag.txt";
                var fso = new ActiveXObject("Scripting.FileSystemObject");
                
                if (!fso.FileExists(flagFile)) {
                    Application.StatusBar = "无更新";
                    return false;
                }
                ...
                Application.StatusBar = "热重载失败: " + e.message;
                return false;
            }
        };
        ...
        /**
         * 注册快捷键（可选）
         * 将热重载绑定到快捷键，如 Ctrl+Shift+R
         */
        this.setupHotkey = function() {
            try {
                // 注册 Ctrl+Shift+R 执行热重载
                Application.OnKey("+^R", "hotReload");
                
                MsgBox("快捷键已注册:\nCtrl+Shift+R - 热重载", 64, "快捷键");
                
            } catch (e) {
                MsgBox("注册快捷键失败: " + e.message, 16, "错误");
            }
        };
         ...
         * 取消快捷键注册
         */
        this.clearHotkey = function() {
            try {
                Application.OnKey("+^R", "");
                MsgBox("快捷键已取消", 64, "快捷键");
                
            } catch (e) {
                MsgBox("取消快捷键失败: " + e.message, 16, "错误");
            }
        };
        this.As = As;
     ...
     * 
     * 使用方法:
     *   在 WPS 中执行: Run("mJSA880_V1_6_1.hotReload")
     * 
     * 注意:
     *   - 需要先运行 watch-and-reload.js 监控源文件
     *   - 修改源 JS 文件并保存后，xlsm 会自动更新
...
// ==================== 热重载功能 ==// 测试自动粘贴 - Sat Jan 24 16:14:18 CST 2026
