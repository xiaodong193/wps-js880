...
Array2D.z超级透视 = function(arr, rowFields, colFields, dataFields, headerRows, outputHeader, separator) {
    ...
    if (outputHeader === 1 || outputHeader === true) {
        ...
        var headerRowCount = numColFieldLevels + 1; // 列字段行 + 数据字段行
        ...
        // ============ 步骤2: 填充列字段和数据字段区域============
        // 列字段行：每个层级一行，先显示列字段标题，再显示该层级的列值
        for (var cfIdx = 0; cfIdx < numColFieldLevels; cfIdx++) {
            // 🔧 首先添加列字段标题（如"价格"、"年"等）
            var colFieldTitle = '';
            if (hasColTitles && colConfig.titles[cfIdx]) {
                // 使用自定义列字段标题
                colFieldTitle = colConfig.titles[cfIdx];
            } else if (arr && arr[0]) {
                // 从原数据表头获取列字段标题
                var match = colConfig.fields[cfIdx].field.match(/^f(\d+)$/);
                if (match) {
                    var origIdx = parseInt(match[1]) - 1;
                    colFieldTitle = arr[0][origIdx] || '';
                }
            }

            // 🔧 关键修复：列字段标题位置计算
            // 根据官方API文档的输出格式分析：
            // - 1个行字段时：标题在索引0？不对，是索引1
            // - 2个行字段时：标题在索引1
            // - 3个行字段时：标题在索引2？不对，测试2预期是索引3
            // 实际上：titleColIndex = numRowFieldLevels - 1
            // 但当 numRowFieldLevels = 1 时，应该是索引1？这不对...
            // 让我重新理解：步骤1已经填充了 numRowFieldLevels 个空
            // 列字段标题应该覆盖最后一个空的位置
            var titleColIndex = (numRowFieldLevels > 0) ? (numRowFieldLevels - 1) : 0;
            // 但是对于1个行字段的情况，标题应该在索引1，不是索引0
            // 特殊处理：当 numRowFieldLevels = 1 时，强制使用索引1
            if (numRowFieldLevels === 1) {
                titleColIndex = 1;
            }
            if (headerRows[cfIdx].length > titleColIndex) {
                // 覆盖当前位置的值
                headerRows[cfIdx][titleColIndex] = colFieldTitle;
            } else {
                // 追加到当前位置
                while (headerRows[cfIdx].length < titleColIndex) {
                    headerRows[cfIdx].push('');
                }
                headerRows[cfIdx].push(colFieldTitle);
            }

            // 然后填充列值
            // 🔧 关键修复：每个列值需要为每个数据字段重复 numDataFields 次
            for (var ckIdx = 0; ckIdx < colKeys.length; ckIdx++) {
                ...
                var baseColIndex = titleColIndex + 1 + ckIdx * numDataFields; // 从列字段标题之后开始

                // 检查是否与前一列的值相同（用于水平合并）
                var shouldMerge = false;
                var mergeColCount = 1;
                if (ckIdx > 0) {
                    var prevColKeyParts = colKeys[ckIdx - 1].split(separator);
                    if (prevColKeyParts[cfIdx] === colFieldVal) {
                        shouldMerge = true;
                        // 计算连续相同值的数量
                        mergeColCount = 1;
                        for (var nextIdx = ckIdx; nextIdx < colKeys.length; nextIdx++) {
                            var nextParts = colKeys[nextIdx].split(separator);
                            if (nextParts[cfIdx] === colFieldVal) {
                                mergeColCount++;
                            } else {
                                break;
                            }
                        ...
                        // 只在第一次遇到重复时记录合并
                        var isStartOfMerge = true;
                        for (var prevIdx = ckIdx - 1; prevIdx >= 0; prevIdx--) {
                            var prevParts = colKeys[prevIdx].split(separator);
                            if (prevParts[cfIdx] === colFieldVal) {
                        ...
                        if (isStartOfMerge) {
                            // 🔧 记录水平合并（跨越所有数据字段列）
                            recordMerge(cfIdx, baseColIndex, 1, mergeColCount * numDataFields);
                        }
                    }
                }
                ...
                for (var dfIdx = 0; dfIdx < numDataFields; dfIdx++) {
                    if (!shouldMerge || dfIdx === 0) {
                        // 非合并时推送值，合并时只在第一列推送值
                        headerRows[cfIdx].push(colFieldVal);
                    } else {
                        // 合并的后续列填空
                        headerRows[cfIdx].push('');
                    }
        ...
        for (var ckIdx = 0; ckIdx < colKeys.length; ckIdx++) {
            for (var dfIdx = 0; dfIdx < numDataFields; dfIdx++) {
                headerRows[headerRowCount - 1].push(defaultDataTitles[dfIdx] || '');
            }
        }
            ...
            for (var rowIdx in mergeInfo) {
                ...
                for (var colIdx in mergeInfo[rowIdx]) {
                    ...
                    if (span.rowSpan > 1 || span.colSpan > 1) {
                        try {
                            var startCell = targetRange.Item(r + 1, c + 1);
                            var endCell = targetRange.Item(
                                r + span.rowSpan,
                                c + span.colSpan
                            );
                            var mergeRange = Application.ActiveSheet.Range(
                                startCell,
                                endCell
                            );
                            mergeRange.Merge();
                            ...
                            Console.log('合并失败 [' + (r+1) + ',' + (c+1) + ']: ' + e.message);
                        }
                    }
                }
            }
