...
Array2D.z超级透视 = function(arr, rowFields, colFields, dataFields, headerRows, outputHeader, separator) {
    ...
    if (outputHeader === 1 || outputHeader === true) {
        ...
        for (var cfIdx = 0; cfIdx < numColFieldLevels; cfIdx++) {
            ...
            // 🔧 关键修复：列字段标题位置计算
            // 综合所有测试案例分析：
            // - 1个行字段+2个列字段（测试1,4）：标题在索引1
            // - 2个行字段+2个列字段（API示例）：标题在索引1
            // - 2个行字段+1个列字段（测试3）：标题在索引2
            // - 3个行字段+1个列字段（测试2）：标题在索引2
            //
            // 核心规律：当有多个列字段时，它们共享同一列位置（索引1）
            //           当只有1个列字段时，标题在 numRowFieldLevels 的位置
            var titleColIndex;
            if (numColFieldLevels > 1) {
                // 多个列字段：标题在索引1（固定的第2列）
                titleColIndex = 1;
            } else {
                // 单个列字段：标题在 numRowFieldLevels 的位置
                titleColIndex = numRowFieldLevels;
            }
            // 特殊处理：1个行字段时，标题总是在索引1
