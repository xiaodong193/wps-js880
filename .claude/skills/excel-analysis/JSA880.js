...
Array2D.prototype.z筛选 = function(predicate, skipHeader) {
    ...
    // 🔧 v4.1 增强: 支持箭头函数中使用 row.f1, row.f18 等属性
    // 将数组行转换为对象，使 row['f1'] 可访问
    var result = data.filter(function(row) {
        // 如果行是数组且函数是箭头函数（需要对象属性访问），则转换
...
Array2D.z超级透视 = function(arr, rowFields, colFields, dataFields, headerRows, outputHeader, separator) {
    ...
    if (outputHeader === 1 || outputHeader === true || outputHeader === -1) {
        ...
        // 🔧 v4.0 新增：outputHeader = -2 表示不生成表头
 ...
 * z合并单元格 - 智能合并透视表标题单元格
 *
 * 【功能说明】将指定的单元格区域按照策略智能合并，支持透视表标题层级合并
 *
 * 【技术实现】
 * 1. 环境检测：检查是否在WPS环境中运行
 * 2. 参数转换：支持字符串地址（"A1:B2"）或 Range 对象
 * 3. 策略处理：根据strategy参数选择合并算法
 * 4. 智能合并：检测并合并连续相同的标题值
 * 5. 返回结果：返回合并后的 Range 对象，便于链式调用
 *
 * 【使用场景】
 * - 透视表标题智能合并（列标题+行标题）
 * - 创建多级表头合并单元格
 ...
 * @example
 * // 示例1：智能合并透视表标题
 * RngUtils.z合并单元格("A1:D3", "cm");
 *
 * // 示例2：使用英文方法名
 * RngUtils.mergeCells("A1:D3", "cm");
 *
 * // 示例3：向后兼容简单合并
 * RngUtils.z合并单元格("A1:B2");
 *
...
RngUtils.z合并单元格 = function(rng, strategy, options) {
    ...
    // 参数转换：字符串地址转为Range对象
    var rangeObj = typeof rng === 'string' ? Range(rng) : rng;

    // 默认选项
    if (!options) options = {};
    var dataRowStart = options.dataRowStart !== undefined ? options.dataRowStart : 0;
    var skipLastRow = options.skipLastRow !== undefined ? options.skipLastRow : true;

    // 读取区域数据
    var data = rangeTo2DArray(rangeObj);
    var rowCount = data.length;
    var colCount = data[0] ? data[0].length : 0;

    // 计算标题行数（假设最后一行是数据标题行）
    var titleRowCount = options.titleRowCount || Math.max(1, rowCount - dataRowStart);
    if (skipLastRow && titleRowCount > 1) {
        titleRowCount -= 1;  // 最后一行是数据标题行，不参与合并
    }
    ...
    // ===== 列标题合并（横向）=====
    for (var row = 0; row < titleRowCount; row++) {
        var rowData = data[row];
        var startCol = (row === 0) ? 1 : 0;  // Row 0从第2列开始，其他行从第1列开始
        var col = startCol;
        ...
        while (col < colCount) {
            ...
            // 查找连续相同值
            while (col + 1 < colCount && rowData[col + 1] === val) {
                col++;
            }
    ...
    // ===== 行标题合并（纵向）=====
    // 从数据行开始
    for (var col = 0; col < Math.min(2, colCount); col++) {  // 只处理前2列（类别、产品）
        var prevVal = null;
        var startRow = dataRowStart;

        for (var row = dataRowStart; row < rowCount; row++) {
            ...
            if (val === prevVal) {
                ...
                // 值变化，合并之前的
                if (prevVal !== null && startRow < row - 1) {
                    mergeList.push({
                        r1: startRow + 1,
                        c1: col + 1,
                        r2: row,      // row是新的行，不包含在内
                        c2: col + 1
                    });
                }
                prevVal = val;
        ...
        // 处理最后一组
        if (prevVal !== null && startRow < rowCount) {
            mergeList.push({
                r1: startRow + 1,
                c1: col + 1,
                r2: rowCount,
    ...
    // 执行合并
    applyMergeOperations(rangeObj, mergeList);
 ...
 * Range转二维数组
 * @private
 * @param {Range} rangeObj - Range对象
 * @returns {Array} 二维数组
 */
function rangeTo2DArray(rangeObj) {
    var rows = rangeObj.Rows.Count;
    var cols = rangeObj.Columns.Count;
    var data = [];
    for (var i = 1; i <= rows; i++) {
        var row = [];
        for (var j = 1; j <= cols; j++) {
            row.push(rangeObj.Cells(i, j).Value2);
        }
        data.push(row);
    }
    return data;
 ...
 * @private
 * @param {Range} rangeObj - Range对象
 * @param {Array} mergeList - 合并区域列表 [{r1, c1, r2, c2}, ...]
 */
function applyMergeOperations(rangeObj, mergeList) {
    if (!mergeList || mergeList.length === 0) return;

    // 先取消区域内所有现有合并
    for (var i = 1; i <= rangeObj.Rows.Count; i++) {
        for (var j = 1; j <= rangeObj.Columns.Count; j++) {
            var cell = rangeObj.Cells(i, j);
            if (cell.MergeCells) {
                cell.MergeArea.UnMerge();
            }
        }
    ...
    mergeList.sort(function(a, b) {
        if (a.r1 !== b.r1) return b.r1 - a.r1;  // 行从大到小
        return b.c1 - a.c1;                    // 列从大到小
    });

    for (var i = 0; i < mergeList.length; i++) {
        var m = mergeList[i];
        var rng = rangeObj.Worksheet.Range(
            rangeObj.Cells(m.r1, m.c1),
            rangeObj.Cells(m.r2, m.c2)
        );
        rng.Merge();
    }
}
...
/**
 * z取消合并填充单元格 - 取消合并并按要求填充
 *
 * 【功能说明】取消指定区域内的合并单元格，并按要求填充值
 *
 * 【技术实现】
 * 1. 环境检测：检查是否在WPS环境中运行
 * 2. 参数转换：支持字符串地址（"A1:B2"）或 Range 对象
 * 3. 扫描所有合并区域，记录左上角单元格的值
 * 4. 调用UnMerge()取消合并
 * 5. 根据 fillAllRows 填充值
 *
 * 【使用场景】
 * - 编辑已合并的透视表标题
 * - 取消合并以便修改数据
 ...
 * @param {Boolean} [fillAllRows=true] - 是否填充所有行
 *                                        true:  所有行都填充原值
 *                                        false: 仅首行填充
 *
 * @example
 * // 取消合并并填充所有单元格
 * RngUtils.z取消合并填充单元格("A1:D3");
 *
 * // 取消合并，仅首行填充
 * RngUtils.z取消合并填充单元格("A1:D3", false);
 *
 * // 使用英文方法名
 * RngUtils.unMergeCells("A1:D3");
 */
RngUtils.z取消合并填充单元格 = function(rng, fillAllRows) {
    // 环境检测：非WPS环境直接返回
    if (!isWPS) return;
    ...
    for (var i = 1; i <= rangeObj.Rows.Count; i++) {
        for (var j = 1; j <= rangeObj.Columns.Count; j++) {
            ...
            // 记录左上角单元格的值
            var topLeft = mergeArea.Cells(1, 1);
            var value = topLeft.Value2;
            ...
            if (fillAllRows) {
                ...
                // 仅左上角填充
                mergeArea.Cells(1, 1).Value2 = value;
                // 其他单元格清空
                for (var r = 1; r <= mergeArea.Rows.Count; r++) {
                    for (var c = 1; c <= mergeArea.Columns.Count; c++) {
                        if (r !== 1 || c !== 1) {
                            mergeArea.Cells(r, c).Value2 = '';
                        }
                    }
                }
            }
 ...
 * $.mergeCells - 智能合并单元格
 * @param {Range|string} rng - 区域
 * @param {string} [strategy] - 合并策略 ('cm'=列层级合并, 'c'=按列, 'r'=按行, 'rm'=先行后列)
 * @param {Object} [options] - 可选配置
 * @returns {RangeChain} RangeChain对象
 * @example
 * $.mergeCells("A1:D3", "cm");  // 智能合并透视表标题
 * $.mergeCells("A1:B2");        // 简单合并（向后兼容）
 */
$.mergeCells = function(rng, strategy, options) {
    var result = RngUtils.mergeCells.apply(RngUtils, arguments);
    if (result && result.Address && typeof result.Address === 'function') {
        return new RangeChain(result);
    }
    return result;
...
/**
 * $.z合并单元格 - mergeCells的中文别名
 */
$.z合并单元格 = $.mergeCells;

/**
 * $.unMergeCells - 取消合并并填充单元格
 * @param {Range|string} rng - 区域
 * @param {boolean} [fillAllRows=true] - 是否填充所有单元格
 * @returns {undefined}
 * @example
 * $.unMergeCells("A1:D3");       // 取消合并并填充所有单元格
 * $.unMergeCells("A1:D3", false); // 取消合并，仅首行填充
 */
$.unMergeCells = function(rng, fillAllRows) {
    RngUtils.unMergeCells.apply(RngUtils, arguments);
};

/**
 * $.z取消合并填充单元格 - unMergeCells的中文别名
 */
