    ...
    for (var r = 0; r < rowValues.length; r++) {
        ...
        for (var c = 0; c < colValues.length; c++) {
            ...
            var value = pivotData[rowKey] && pivotData[rowKey][colKey] !== undefined
                ...
                : (aggregator === function(acc, val) { return (isNaN(acc)?0:acc) + (isNaN(val)?0:val); } ? 0 : '');
            rowData.push(value);
        }
        result.push(rowData);
    }
 ...
 * @returns {Sheet|null} 工作表对象，失败时返回null
 * @example
 * asSheet("1月")           // 工作表对象
 * asSheet(1)              // 第一个工作表
 * asSheet(Sheets(1))       // 返回原工作表对象
 */
function asSheet(sht) {
    ...
    // 如果已经是Sheet对象，直接返回
    if (sht && sht.Activate && sht.Name) return sht;
    ...
    if (typeof sht === 'string') {
        try {
            ...
            try {
                ...
                console.log("asSheet模糊匹配失败: " + e2.message);
            }
            return null;
        }
    }
 ...
 * @param {number} [dimension=1] - 要获取上界的维度（默认为1）
 * @returns {Number} 指定维度的上界
 * @example
 * ubound([[1,2,3],[4,5,6]], 2)  // 2 (第二维上界，实际列数-1)
 */
const ubound = (arr, dimension = 1) => {
